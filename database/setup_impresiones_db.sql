/*
  # Complete Database Setup for Print Management System (impresiones_db)
  
  This script creates a local PostgreSQL database with all necessary tables,
  functions, indexes, and sample data for the print management system.
  
  Database: impresiones_db
  
  Tables:
  - users: System users with account information
  - prints_raw: Raw data from each CSV import
  - prints_monthly: Monthly aggregates per user
  - import_log: Import audit log
  - printers: Printer management
  - user_printer_assignments: Many-to-many relationship between users and printers
  
  Usage:
  1. Create database: CREATE DATABASE impresiones_db;
  2. Connect to database: \c impresiones_db
  3. Run this script: \i setup_impresiones_db.sql
*/
...
DROP FUNCTION IF EXISTS total_by_user(jsonb) CASCADE;
...
-- Monthly aggregated prints data
...
-- Sample user-printer assignments
...
BEGIN
  ...
  RAISE NOTICE 'Printers inserted: %', (SELECT COUNT(*) FROM printers);
  RAISE NOTICE '============================================';
END $$;
